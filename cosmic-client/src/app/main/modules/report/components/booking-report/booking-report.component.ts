...
export class BookingReportComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    displayedColumns = ['customer', 'staff', 'date', 'time', 'services', 'price', 'status'];
    ...
    submit(event: MouseEvent, download: boolean): void {
        ...
        this._reportService.getBookingReportData(sendObj, download)
            .pipe(
                takeUntil(this._unsubscribeAll),
                finalize(() => {
                    this.tableLoading = false;
